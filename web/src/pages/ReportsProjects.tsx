...
export default function ReportsProjects() {
  ...
  const { data: reports = [], isLoading } = useQuery({
    ...
    queryFn: () => getReports('projects'),
    refetchInterval: 60_000,
  })

  const hasReport = reports.length > 0
  const iframeUrl = '/outputs/projects_latest.html'
        ...
        <div>
          ...
          {hasReport && (
            <span style={{ color: '#8b949e', fontSize: 12, marginLeft: 12 }}>
              最新：{reports[0].period_key} · {reports[0].item_count} 个项目
            </span>
          )}
        </div>
        <Space>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '100%', gap: 16 }}>
            ...
            <div style={{ color: '#8b949e', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 13 }}>
                系统每天 06:00 自动生成。可先运行抓取后手动触发：
              </div>
            </div>
            <Alert
              type="info"
              showIcon
              message={<code style={{ fontSize: 12 }}>radar crawl github && radar score</code>}
